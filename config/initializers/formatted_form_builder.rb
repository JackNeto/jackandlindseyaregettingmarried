class FormattedFormBuilder < ActionView::Helpers::FormBuilder
  ...
  def radio_button(method, tag_value, options = {})
    if options && options[:choices]
      radios = options.delete(:choices).collect{|choice| %{<div class="radio_button">}+super(method, choice[0], options) + %{<label for="#{object_name.to_s.gsub(']', '').gsub('[', '_')}_#{method}_#{choice[0]}">#{choice[1]}</label></div>}}.join
      standard_field('radio_button', method, options) { radios }
    elsif options && options[:value_name]
        standard_field('radio_button', method, options) { super(method, tag_value) + %{<label for="#{object_name}_#{method}_#{tag_value}">#{options[:value_name]}</label><div class="clearfloat"></div>}}
    else
  ...
  def error_messages
    if @object && !@object.errors.empty?
      message = @object.errors[:base] ? @object.errors[:base]: 'There were some problems submitting this form. Please correct all the highlighted fields and try again'
  ...
  def wgtg_fields_for(record_or_name_or_array, *args, &block)
    options = args.extract_options!
    options.merge!(:builder => WgtgFormBuilder)
